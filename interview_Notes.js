...
// 2- find out if a given word is a palindrome (without using the reverse method). kayak mom racecar
...
// 	ie. findLongestSeq([3,3,4,4,4,5,6,6,6,1,1,4,4,4,1,1,1,1,1,4,4,4,4,6]) = 1
...
// In JavaScript, source prototypes are commonly referred to as mixins. Since ES6, this feature has a convenience utility in JavaScript called `Object.assign()`. 
...
compare str[0] with str[6]
compare str[1] with str[5]
compare str[2] with str[4]

compare str[3] with str[3]
compare str[4] with str[2]
compare str[5] with str[1]
compare str[6] with str[0]

*/

function reverse(word) {
	let str = word.split('') // str is a poorly named array
	const strLength = str.length - 1
	for (let i = 0 ; i <= strLength/2; i++) {
		[str[i], str[strLength - i ]] = [str[strLength - i ], str[i]]
	}
	return str.join('')
}
...
[3,3,4,4,4,5,6,6,6,1,1,4,4,4,1,1,1,1,1,4,4,4,4,6]
...
function findMostRepeated(arr) {
	...
	for ( let i=0; i<arr.length; i++ ) {
		let num = arr[i]
		if (obj[num]) {
			// arr[i] is already in the object, increase
			obj[num]++
		} else {
			// not in obj
			obj[num] = 1
